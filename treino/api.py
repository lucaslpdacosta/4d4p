...
def progresso_aluno(request, email_aluno: str):
    ...
    total_aulas_concluidas_faixa = AulasConcluidas.objects.filter(aluno=aluno).count()
...
def aula_realizada(request, aula_realiza: AulaRealizadaSchema):
    ...
    return 200, f"Aulas marcada como realizada para o aluno {aluno.nome}"
...
def update_aluno(request, aluno_id: int, aluno_data: AlunosSchema):
    ...
    if int(idade.days/365) < 18 and aluno_data.dict()['faixa'] in ('A', 'R', 'M', 'P'):
        raise HttpError (400, 'Menbres de 18 não podem receber essa faixa')
